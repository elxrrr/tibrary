...
def prepare_library(store, root, snapshot, cancel, progress, force=False):
    if not snapshot:
        snapshot = cached_inspection(store, root)
    progress("Checking for local changes before continuing · unchanged tags are reused")
    try:
        result = scan(store, root, cancelled=cancel, progress=progress, force=force)
    except OSError:
        if snapshot:
    ...
    if result.get("status") != "complete":
        if snapshot:
            return list(snapshot)
        raise ValueError(
            "Local refresh did not complete; the previous preview is retained. Reconnect the drive or retry."
        )
    try:
        paths = [
            r["path"]
            for r in store.rows(
                "SELECT path FROM local_files WHERE root=? AND present=1 AND lower(path) LIKE '%.flac'",
                (str(root),),
            )
        ]
        rows = inspect_snapshot(
            root,
            () if force else snapshot,
            cancel,
            progress,
            store.preferences("organisation"),
            paths=paths,
        )
        ...
        raise
    if not cancel():
        save_inspection(store, root, rows)
    for row in rows:
        if first(row.get("tags", {}), "albumartist") in result.get(
            "changed_artists", ()
        ):
            row["needs_artist_match"] = True
    return rows
...
def cached_inspection(store, root):
    ...
    for source in data.get("rows", []) if data.get("version") == 1 else []:
        ...
        if row.get("cover_size"):
            row["cover_size"] = tuple(row["cover_size"])
        rows.append(row)
    return rows
...
def save_inspection(store, root, rows):
    ...
    store.save_preferences(
        ...
        dict(
            version=1,
            rows=[
                {key: row[key] for key in INSPECTION_FIELDS if key in row}
                for row in rows
            ],
        ),
    )
