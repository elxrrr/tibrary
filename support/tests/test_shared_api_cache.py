...
class SharedCacheTests(unittest.TestCase):
    def setUp(self):
        ...
        self.details = dict(
            ...
            tracks_loaded=True,
            item_count=2,
            tracks=[dict(id="11", track_number=1), dict(id="12", track_number=2)],
        )
    ...
    def test_cross_instance_cache_is_detached_and_market_scoped(self):
        with patch.object(Tidal, "release_details", return_value=self.details) as fetch:
            first = self.api.release_details(self.release)
            first["tracks"].clear()
    ...
    def test_changed_summary_expiry_and_force_refresh(self):
        with patch.object(Tidal, "release_details", return_value=self.details) as fetch:
            ...
            saved = self.store.preferences("tag-review:GB:1")
            saved["details_checked_at"] = time.time() - 86400 * 31
    ...
    def test_failure_preserves_cache_and_details_do_not_refresh_credit_age(self):
        saved = dict(
            self.details, tag_checked_at=1, tag_credits_checked=True, metadata_schema=3
        )
        self.store.save_preferences("tag-review:GB:1", saved)
        with patch.object(
            Tidal, "release_details", side_effect=RuntimeError("offline")
        ):
            with self.assertRaises(RuntimeError):
                self.api.release_details(self.release)
        self.assertEqual(self.store.preferences("tag-review:GB:1"), saved)
        with patch.object(Tidal, "release_details", return_value=self.details):
            self.api.release_details(self.release)
    ...
    def test_album_and_track_metadata_reused(self):
        with (
            patch.object(
                Tidal,
                "album_tag_details",
                return_value=dict(self.details, tag_credits_checked=True),
            ),
            patch.object(
                Tidal,
                "track_tag_details",
                return_value={"id": "11", "bpm": 125, "key": "C"},
            ) as track,
        ):
        ...
        with patch.object(
            Tidal, "album_tag_details", side_effect=AssertionError("cached")
        ):
            self.assertTrue(
                other.album_tag_details(self.release)["tag_credits_checked"]
            )

    def test_searches_editions_and_discography_shared_until_refresh(self):
        for method, args in [
            ("search", ("Artist",)),
            ("search_albums", ("Release",)),
            ("search_tracks", ("Song",)),
            ("recording_releases", ("ISRC",)),
        ]:
            with (
                self.subTest(method=method),
                patch.object(Tidal, method, return_value=[{"id": "1"}]) as fetch,
            ):
                getattr(self.api, method)(*args)
                other = CachedTidal("GB", store=self.store)
                self.assertEqual(getattr(other, method)(*args), [{"id": "1"}])
                self.assertEqual(fetch.call_count, 1)
        with (
            patch.object(
                Tidal, "artist", return_value={"id": "1", "releases": [self.release]}
            ) as artist,
            patch.object(
                Tidal, "release_details", return_value=self.details
            ) as details,
        ):
            self.api.artist("1", detailed=True)
            self.api.artist("1", detailed=True)
            self.api.artist("1", detailed=True, force=True)
            self.assertEqual(artist.call_count, 2)
            self.assertEqual(details.call_count, 1)

    def test_availability_keeps_release_identity_and_reuses_observation(self):
        with patch.object(Tidal, "release_availability", return_value=dict(self.release, available=True)) as fetch:
            self.api.release_availability(self.release)
            saved = self.store.preferences("tag-review:GB:1")
            self.assertEqual(saved["id"], "1")
            self.assertEqual(saved["title"], "Album")
            other = CachedTidal("GB", store=self.store)
            self.assertTrue(other.release_availability(self.release)["available"])
            self.assertEqual(fetch.call_count, 1)
