...
use crate::stream_download::{
    self, apply_audio_tags, download_stream, fetch_album_info, fetch_album_tracks,
    fetch_cover_art, fetch_lyrics, format_download_path, get_playback_info,
    get_valid_token, publish_staged_files, TrackDownloadMeta,
};
...
impl DownloadManager {
    pub async fn run_downloads(
        ...
        let skip_existing = settings.get("skip_existing").and_then(|v| v.as_bool()).unwrap_or(true);
        let lyrics_embed = settings.get("lyrics_embed").and_then(|v| v.as_bool()).unwrap_or(true);
        let lyrics_file = settings.get("lyrics_file").and_then(|v| v.as_bool()).unwrap_or(false);
        let cover_album_file = settings.get("cover_album_file").and_then(|v| v.as_bool()).unwrap_or(true);
        let playlist_create = settings.get("playlist_create").and_then(|v| v.as_bool()).unwrap_or(false);
        let replay_gain = settings.get("replay_gain").and_then(|v| v.as_bool()).unwrap_or(true);

        let provider = db.get_preference("provider").await?.unwrap_or_else(|| json!({}));
        let download_delay = provider.get("download_delay").and_then(|v| v.as_bool()).unwrap_or(true);
        let delay_min = provider.get("download_delay_min_sec").and_then(|v| v.as_f64()).unwrap_or(3.0);
        let delay_max = provider.get("download_delay_max_sec").and_then(|v| v.as_f64()).unwrap_or(5.0);
        ...
        let template = layout
            .get("template")
            .and_then(|v| v.as_str())
            .unwrap_or("{album_artist}/{album}/{track_number} {title}");
        ...
        let token = match get_valid_token(db, &http).await {
            Ok(tok) => tok,
            Err(_) => {
                ...
                progress_cb("Authentication required. Please complete sign-in in your browser.".to_string());
                return Err("Tidal sign-in required. Follow the link to log into Tidal.".to_string());
            }
        };

        progress_cb("Tidal account connected · lossless audio · sequential track downloads".to_string());
        ...
        for (ident, release) in &items {
            ...
            let album_info = match fetch_album_info(&http, ident, &token).await {
                Ok(info) => info,
                Err(e) => {
                    progress_cb(format!("Failed to fetch album details for {}: {}", ident, e));
                    continue;
                }
            };
            ...
            let all_tracks = match fetch_album_tracks(&http, ident, &token, &cancel_flag).await {
                Ok(t) => t,
                Err(e) => {
                    if cancel_flag.load(Ordering::Relaxed) {
                        break;
            ...
            let selected_ids: Option<std::collections::HashSet<String>> = selected.map(|arr| {
                arr.iter()
                    .filter_map(|s| s.get("id").map(|v| v.to_string().trim_matches('"').to_string()))
                    .collect()
            });
            ...
            for (idx, track) in tracks.iter().enumerate() {
                ...
                let full_album_title = if let Some(ref ver) = album_info.version {
                    if !ver.is_empty() && !album_info.title.to_lowercase().contains(&ver.to_lowercase()) {
                ...
                if skip_existing {
                    let rel_flac = if let Some(dest) = destination {
                        let rel_album = dest.get("album_relative").and_then(|v| v.as_str()).unwrap_or("");
                        let disc_dir_name = if disc_total > 1 { format!("Disc {}", track.volume_number) } else { String::new() };
                        let filename = format_download_path("{track_number} {title}", &prelim_meta, ".flac");
                        let mut p = PathBuf::from(rel_album);
                        if !disc_dir_name.is_empty() { p.push(disc_dir_name); }
                        p.push(filename);
                        p
                    } else {
                    ...
                    let exists_flac = dst_flac.is_file() && dst_flac.metadata().map(|m| m.len() > 0).unwrap_or(false);
                    let exists_m4a = dst_m4a.is_file() && dst_m4a.metadata().map(|m| m.len() > 0).unwrap_or(false);

                    if exists_flac || exists_m4a {
                        progress_cb(format!("Track {}/{} · {} (already exists, skipped)", idx + 1, total_tracks, track.title));
                ...
                progress_cb(format!("Track {}/{} · {}", idx + 1, total_tracks, track.title));

                let stream_info = match get_playback_info(&http, &track.id, &token, item_quality).await {
                    Ok(s) => s,
                    Err(e) => {
                ...
                let staged_source = stage_dir.join(format!(".source-{}{}", track.id, ext));
                ...
                if let Err(e) = download_stream(&http, &stream_info, &staged_source, &cancel_flag, &p_cb).await {
                    if cancel_flag.load(Ordering::Relaxed) {
                        all_tracks_ok = false;
                        break;
                    }
                ...
                // Detect actual container format from magic bytes
                let detected_ext = if let Ok(mut f) = fs::File::open(&staged_source) {
                    use std::io::Read;
                    let mut header = [0u8; 16];
                    let n = f.read(&mut header).unwrap_or(0);
                    stream_download::detect_audio_extension(&header[0..n], ext)
                } else {
                    ext.to_string()
                };

                // Fetch lyrics
                let (lyrics_synced, lyrics_unsynced) = fetch_lyrics(&http, &track.id, &token).await;
                ...
                let meta = TrackDownloadMeta {
                    ...
                    release_type: None,
                    explicit: track.explicit,
                    lyrics: if lyrics_embed { lyrics_synced.clone() } else { None },
                    unsynced_lyrics: if lyrics_embed { lyrics_unsynced.clone() } else { None },
                    album_replay_gain: if replay_gain { stream_info.album_replay_gain } else { None },
                    album_peak_amplitude: if replay_gain { stream_info.album_peak_amplitude } else { None },
                    track_replay_gain: if replay_gain { stream_info.track_replay_gain } else { None },
                    track_peak_amplitude: if replay_gain { stream_info.track_peak_amplitude } else { None },
                };

                if let Err(e) = apply_audio_tags(&staged_source, &meta, cover_data.as_deref()) {
                    progress_cb(format!("Tagging warning for {}: {}", track.title, e));
                }
                ...
                let target_path_in_stage = if let Some(dest) = destination {
                    ...
                    let filename = format_download_path("{track_number} {title}", &meta, &detected_ext);
                ...
                let _ = fs::rename(&staged_source, &target_path_in_stage);
            ...
            fn find_first_dir(dir: &Path) -> Option<PathBuf> {
            ...
            let album_dir_opt = find_first_dir(&stage_dir);
            ...
            if playlist_create && !relative_track_filenames.is_empty() {
                if let Some(ref album_dir) = album_dir_opt {
                    let _ = stream_download::write_m3u8_playlist(album_dir, "_playlist", &relative_track_filenames);
                }
            }
